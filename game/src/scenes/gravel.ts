...
//Custom Interfaces
import { InitialPlayerInformations, Chat, RoomInfoGame } from '../interfaces/interfaces'
...
export default class Gravel extends Phaser.Scene {
  ...
  addOtherPlayer(id: string, playerInfo: any) {
  ...
  addPlayer(id: string, playerInfo: any) {
  ...
  loadChat (chat: Chat[]) {
    ...
    let gameChatChild = this.gameChat.getChildByID('global_chat') as any;
  ...
  create() {
    ...
    this.socket.on('currentPlayers', ({players, chat}: InitialPlayerInformations) => {
      Object.keys(players).forEach( (id) => {
        if (id == this.socket.id && !(this.socket.id in allPlayers)) {
    ...
    this.socket.on('playerMoved', (players: any) => {
      ...
      Object.keys(players).forEach( (id) => {
        ...
        if (id !== this.socket.id) {
          ...
          if (allPlayers[id].anims.getName() !== 'death') {
            // if player died, not repeat the animation of death every time
